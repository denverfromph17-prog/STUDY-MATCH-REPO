...
const jpeg=Buffer.from([0xff,0xd8,0xff,0xe0,0,0,0,0,0xff,0xd9]);
...
test('corrupt image with only a valid signature is rejected',async()=>{const corrupt=Buffer.from([0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a,0,0,0,0]);const r=await agent.post('/api/profile/photo').attach('photo',corrupt,{filename:'double.jpg.png',contentType:'image/png'});assert.equal(r.status,400);assert.equal(readdirSync(uploadDir).length,0);});
