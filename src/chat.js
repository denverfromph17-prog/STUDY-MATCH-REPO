...
function authorizedConversation(db,conversationId,userId){return db.prepare(`
  ...
  WHERE c.id=? AND (c.user_one_id=? OR c.user_two_id=? ) AND NOT EXISTS(SELECT 1 FROM blocked_users b WHERE (b.blocker_user_id=c.user_one_id AND b.blocked_user_id=c.user_two_id) OR (b.blocker_user_id=c.user_two_id AND b.blocked_user_id=c.user_one_id))
...
export function registerChatRoutes(app,{db,config,now}){
  ...
  app.post('/api/conversations/open/:userId',auth,(req,res,next)=>isBlocked(db,req.user.id,req.params.userId)?res.status(403).json({error:'Private chat is unavailable.'}):next());
  ...
  app.get('/api/conversations',auth,(req,res)=>{const rows=db.prepare(`
    ...
    JOIN profiles p ON p.user_id=CASE WHEN c.user_one_id=? THEN c.user_two_id ELSE c.user_one_id END
    WHERE (c.user_one_id=? OR c.user_two_id=?) AND NOT EXISTS(SELECT 1 FROM blocked_users b WHERE (b.blocker_user_id=c.user_one_id AND b.blocked_user_id=c.user_two_id) OR (b.blocker_user_id=c.user_two_id AND b.blocked_user_id=c.user_one_id)) ORDER BY COALESCE(last_message_at,c.updated_at) DESC,c.id
